...
Example: ~/.ssh/jetstream_aws.pub
DESCRIPTION
}

variable "key_name" {
  default     = "jetstream-benchmark-key"
...
resource "aws_instance" "jetstream" {
  ami                    = var.ami
  instance_type          = var.instance_types["jetstream"]
  key_name               = aws_key_pair.auth.id
  subnet_id              = aws_subnet.benchmark_subnet.id
  vpc_security_group_ids = [aws_security_group.benchmark_security_group.id]
  count                  = var.num_instances["jetstream"]

  tags = {
    Name = "jetstream_${count.index}"
  }
}
...
resource "aws_instance" "client" {
  ...
  tags = {
    Name = "jetstream_client_${count.index}"
  }
}
