...
resource "alicloud_instance" "nsq" {
  provider          = "alicloud"
  ...
  instance_type        = "${var.instance_types["nsq"]}"
  security_groups      = ["${alicloud_security_group.default.id}"]
  vswitch_id = "${alicloud_vswitch.vswitch.id}"
  internet_max_bandwidth_out = "1"
  system_disk_category = "cloud_ssd"
  system_disk_size = "500"
  count = "${var.num_instances["nsq"]}"
  key_name = "${alicloud_key_pair.key_pair.id}"
  instance_name        = "nsq-${count.index}"
  password = "Ali123456"
}
...
resource "alicloud_instance" "client" {
  ...
  key_name = "${alicloud_key_pair.key_pair.id}"
  system_disk_category = "cloud_ssd"
  instance_name        = "nsq-client-${count.index}"
}
...
output "broker_host" {
  value = "${alicloud_instance.nsq.*.public_ip}"
}
